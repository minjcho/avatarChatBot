...
class SynthesizedAudioEmitter:
    ...
    def flush(self):
        """모은 프레임을 한 번에 합쳐서 output.wav 로 저장하고, 이벤트로도 전송"""
        if not self._frames:
            return
        ...
        # 2) 이벤트 채널로 전송 (is_final=True 로 표시)
        audio_evt = SynthesizedAudio(
            frame=combined,
        ...
        self._event_ch.send_nowait(audio_evt)

        # 3) wav 바이트 생성 후 파일 저장
        Path("./wav").mkdir(parents=True, exist_ok=True)
        Path("./wav/output_temp.wav").write_bytes(combined.to_wav_bytes())
        # change name output_temp.wav to output.wav with os
        import os
        os.rename("./wav/output_temp.wav", "./wav/output.wav")

        # 4) 다음 세그먼트를 위해 리스트 비우기
        self._frames.clear()
