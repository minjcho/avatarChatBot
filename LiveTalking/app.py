# ─── 위쪽 import 블록 아무 곳에 추가 ───────────────────────────────
import os, time          # ← NEW (siren 파일 감시용)
...
# ─── add near other helper defs (e.g., after post/run helpers) ─────
SIREN_FILE = "./avatar/wav/output.wav"

def _watch_siren_file():
    """
    Poll SIREN_FILE; if mtime changes (or file appears) read bytes
    and push them into the first active session’s `put_audio_file`.
    Runs as a daemon thread when --siren is given.
    """
    last_mtime = None
    while True:
        # print(f"watching {SIREN_FILE}")
        if os.path.exists(SIREN_FILE):
            # print(f"found {SIREN_FILE}")
            mtime = os.path.getmtime(SIREN_FILE)
            if last_mtime is None or mtime != last_mtime:
                print(f"new siren file: {SIREN_FILE} mtime={mtime}")
                last_mtime = mtime
                with open(SIREN_FILE, "rb") as f:
                    data = f.read()
                # pick the first active session
                for sid, real in nerfreals.items():
                    if real is not None:
                        try:
                            real.put_audio_file(data)
                            logger.info(f"[siren] pushed new audio to session {sid}")
                        except Exception as e:
                            logger.warning(f"[siren] push failed: {e}")
                        break
        time.sleep(0.0001)   # 4 Hz polling; tweak as you like
...
if __name__ == '__main__':
    ...
    parser.add_argument('--siren', action='store_true',
                    help='watch /home/davin/LiveTalking/avatar/wav/output.wav and feed it to humanaudio automatically')
    ...
    def run_server(runner):
        ...
        if opt.siren:
            Thread(target=_watch_siren_file, daemon=True).start()
